...
img = cv.imread('assignment_01_images/spider.png', cv.IMREAD_COLOR)
...
for a in a_val:
    func = np.minimum(s + a * 128 * np.exp(-(s - 128) ** 2 / (2 * sigma ** 2)), 255).astype(np.uint8)
...
plt.tight_layout()
plt.savefig('Q3/a_value_var.jpg')
plt.show()